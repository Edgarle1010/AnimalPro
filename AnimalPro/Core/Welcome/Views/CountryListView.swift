        ...
        VStack {
            ...
            List {
                ForEach(countryDictionary.sorted(by: <), id: \.key) { key , value in
                    if let countryName = countryName(countryCode: key), (countryName.lowercased().contains(searchText.lowercased()) || searchText == "") {
                        HStack(spacing: 15) {
...
struct CountryListView_Previews: PreviewProvider {
    static var previews: some View {
        CountryListView(countryCode: .constant("+52"), countryFlag: .constant(""))
    }
}
